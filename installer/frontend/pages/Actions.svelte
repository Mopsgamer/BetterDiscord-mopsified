<script>
    ...
    import {action, platforms} from "../stores/installation";

    let group = $action;
    canGoBack.set(true);

    // Dynamic label based on whether any selected platform is already injected
    $: isAnyInjected = $platforms.some(p => p.isInjected);
    $: installLabel = isAnyInjected ? "Reinstall BetterDiscord" : "Install BetterDiscord";
...
<section class="page" in:page out:page="{{out: true}}">
    ...
    <RadioGroup index={$radioSelectedIndex}>
        ...
        <Radio bind:group on:change={() => update(1)} value="uninstall">
