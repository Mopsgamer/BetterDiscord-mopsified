...
export default async function (installations) {
  ...
  const appData = await ipcRenderer.invoke("get-app-data-path");
  ...
  const progressPerLoop =
    (INJECT_PROGRESS - progress.value) / installations.length;
  ...
  for (const inst of installations) {
    ...
    try {
      ...
      injectionProcess.addEventListener("copy", (ev) =>
        log(
          `\x1b[36mCopying:\x1b[0m ${ev.detail.source} -> ${ev.detail.destination}`,
        ),
      );
      ...
      log(
        `\x1b[31mFailed to inject into ${inst.channel}: ${err.message}\x1b[0m`,
      );
      return fail();
    }
  }
