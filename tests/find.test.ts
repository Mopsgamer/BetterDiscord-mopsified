...
import { find, findNow } from "../packages/find/src/index";
...
test("findNow should find modules instantly", () => {
	const results = findNow([(m) => m.createElement && m.useLayoutEffect, (m) => m.dispatch]);
...
test("find should batch calls", async () => {
	const p1 = find([(m) => m.createElement]);
	const p2 = find([(m) => m.dispatch]);
