...
async function downloadAndUnpack(channel: string) {
    const TEMP_DIR = path.relative(process.cwd(), path.join(import.meta.dirname, "..", "temp", channel));
    ...
    try {
        ...
        console.log(c("cyan", `[${channel}] `) + `Extracting archive to '${EXTRACT_DIR}'...`);
        ...
        if (!fs.existsSync(discordDir)) {
            ...
            console.log(c("cyan", `[${channel}] `) + `Getting discord's help in '${exePath}'...`);
            await $`${exePath} --help`.quiet().nothrow();
        }
