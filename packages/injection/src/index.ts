...
export function getInstallations(options: InjectionOptions = {}): DiscordInstallation[] {
    ...
    for (const channel of channels) {
        ...
        if (process.platform === "win32") {
            ...
            // Linux
            if (options.flatpak) {
                discordDir = path.join(process.env.HOME!, ".var", "app", "com.discordapp.Discord", "config", nameNoSpace.toLowerCase());
            } else {
...
function createInstallation(channel: DiscordRelease, version: string, resourcesPath: string, corePath: string): DiscordInstallation {
    return {
        channel,
        version,
        resourcesPath,
...
export async function inject(inst: DiscordInstallation, options: InjectionOptions = {}): Promise<void> {
...
async function patchAsar(resourcesPath: string): Promise<void> {
    ...
    if (fs.existsSync(targetFile)) {
        ...
        fileContent = fileContent.replace(/(protocol\.registerSchemesAsPrivileged\(\s*\[)(\s*{\s*scheme:\s*)/, `$1${patch}$2`);
